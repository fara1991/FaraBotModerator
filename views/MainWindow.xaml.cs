    ...
    public partial class MainWindow
    {
        ...
        private async Task StartTimer()
        {
            ...
            while (true)
            {
                ...
                // Interval Timer
                if (CycleTimer1CheckBox.IsChecked ?? false) timer1Count++; else timer1Count = 0;
                if (CycleTimer2CheckBox.IsChecked ?? false) timer2Count++; else timer2Count = 0;
                if (CycleTimer3CheckBox.IsChecked ?? false) timer3Count++; else timer3Count = 0;
                if (CycleTimer4CheckBox.IsChecked ?? false) timer4Count++; else timer4Count = 0;
        ...
        private void FixedTimerMessage(DateTime? date, DateTime? time, string message)
        {
            ...
            var datetime = new DateTime(date.Value.Year, date.Value.Month, date.Value.Day, time.Value.Hour, time.Value.Minute, time.Value.Second);
            if (DateTime.Now >= datetime && (DateTime.Now - datetime).TotalSeconds < 1) _twitchClientController?.SendModeratorMessage(message);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void FaraBotModeratorWebView_NavigationCompleted(object? sender, Microsoft.Web.WebView2.Core.CoreWebView2NavigationCompletedEventArgs e)
        {
